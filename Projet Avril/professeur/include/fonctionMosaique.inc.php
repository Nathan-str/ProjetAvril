			...
			if ($choose == true){
				...
				if (!isset($jsonArray["Error"])){
					...
					for ($i=1; $i <= sizeof($jsonArray["$filiere"]); $i++){
						...
						echo("<img src=\"http://nathan-str-etudiant.alwaysdata.net/images/" . $jsonArray["$filiere"][$i]['image'] . "\" width=\"200\" height=\"200\" style=\"border-radius:100px;\" alt=\"error\" class=\"image\" onclick=\"clickImage($i);\" /><br />");
						echo "<p style=\"color: #f7f1e3;\">".$jsonArray["$filiere"][$i]['prenom'] . " " . $jsonArray["$filiere"][$i]['nom'] . "</p><br />";
						echo "<p class=info id=$i style=\"display: none; color:#f7f1e3;\">".$jsonArray["$filiere"][$i]['mail'] . "<br />". $jsonArray["$filiere"][$i]['numero'] . "</p><br />";
						echo("</div>");	
				...
				if (!isset($jsonArray["Error"])){
					...
					for ($i=1; $i <= sizeof($jsonArray["$filiere"]["$groupe"]); $i++){
						...
						echo("<div class=\"profil\">");
						echo("<img src=\"http://nathan-str-etudiant.alwaysdata.net/images/" . $jsonArray["$filiere"]["$groupe"][$i]['image'] . "\" width=\"200\" height=\"200\" style=\"border-radius:100px;\" alt=\"error\" class=\"image\" onclick=\"clickImage($i);\" /><br />");
						echo "<p style=\"color: #f7f1e3;\">".$jsonArray["$filiere"]["$groupe"][$i]['prenom'] . " " . $jsonArray["$filiere"]["$groupe"][$i]['nom'] . "</p><br />";
						echo "<p class=\"info\" id=\"$i\" style=\"display: none; color:#f7f1e3;\">".$jsonArray["$filiere"]["$groupe"][$i]['mail'] . "<br />". $jsonArray["$filiere"]["$groupe"][$i]['numero'] . "</p><br />";
						echo("</div>");	
